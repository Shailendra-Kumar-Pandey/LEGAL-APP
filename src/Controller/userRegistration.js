...
export const Ragistration = async (req, res) => {
  ...
  if (!email.includes('@' && '.com')) {
    return res.status(400).json({success:false, message: "Please Enter Vailid Email..." });
  }

  // Chack Database inside Email is unique
  if(await user.findOne({email:email})){
    return res.status(400).json({success:false,message:"Email Already Exit..."})
  }
  ...
  try {
    ...
    return res.status(500).json({success:false, message: `${error} Somthing went wrong...` });
  }
};
...
export const Login = async (req, res)=>{
  ...
  try {
    ...
    if(!existingUser){
      return res.status(404).json({success : false, message : "Invailid Email..."})
    }
    ...
    if(!isMatch){
      return res.status(404).json({success : false, message : "Invailid Password..."})
    }
    ...
    if(existingUser.role === 'LAWYER'){
      ...
      if(!lawyer){
        return res.status(404).json({success : false, message: 'Please complte Profile..'})
      }
  
      if(lawyer?.status !== 'APPROVED'){
        return res.status(404).json({success : false, message: `Status: ${lawyer.status}, Please contact tha Admin...`})
      }
